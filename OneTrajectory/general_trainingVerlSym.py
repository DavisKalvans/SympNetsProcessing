...
def train_model(problem, method, device, data, trainParams, nL, nN, nM, extraParams = None):
    ...
    # x_train = torch.from_numpy(np.float64(npz_file['train_X'])).float().to(device)  
    # y_train = torch.from_numpy(np.float64(npz_file['train_Y'])).float().to(device) 
    # tau_train = torch.from_numpy(np.float64(npz_file['train_Tau'])).float().to(device)
    # x_test = torch.from_numpy(np.float64(npz_file['test_X'])).float().to(device)
    # y_test = torch.from_numpy(np.float64(npz_file['test_Y'])).float().to(device)
    # tau_test = torch.from_numpy(np.float64(npz_file['test_Tau'])).float().to(device)
    ...
    if sch:
        model_name = f'TrainedModels/{method}/Sym/{problem}/sch{problem}RandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_' +f
    else:
        model_name = f'TrainedModels/{method}/Sym/{problem}/{problem}RandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_' +f
...
### Same, but saves the model after each epoch into a list of models
def train_modelSaveAtEachEpoch(problem, method, device, data, trainParams, nL, nN, nM, extraParams = None):
    import os
    ...
    f = str(nL) + "L" + str(nN) + "n" + str(nM) + "m"
    if sch:
        model_name = f'TrainedModels/{method}/Sym/{problem}/SaveAtEpochs/sch{problem}RandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_' +f
    else:
        model_name = f'TrainedModels/{method}/Sym/{problem}/SaveAtEpochs/{problem}RandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_' +f

    if linear:
        model_name = model_name.replace(f'{problem}Rand', f'{problem}LinearRand')

    current_path = os.getcwd()
    folder = current_path + '/' + model_name
...
def model_errors(problem, method, device, data, trainParams, nL, nN, nM, plotParams, extraParams = None):

    ### Process the provided parameters
    N_train, M_test, tau = data
    tau_txt = str(tau).replace('.', '')

    M, tau, x0, best = plotParams
    d = len(x0)
    D = int(d/2)
    Tend = tau*M
    ...
    ### Plot the errors
    if linear:
        plot_name = model_name.replace(f'{method}/Sym/{problem}/', f'{method}/Sym/{problem}/Predictions/Linear/T{Tend_txt}/')
    else:
        plot_name = model_name.replace(f'{method}/Sym/{problem}/', f'{method}/Sym/{problem}/Predictions/Sigmoid/T{Tend_txt}/')
...
def model_errorsAtEachEpoch(problem, method, data, trainParams, nL, nN, nM, predParams, skip, extraParams = None): # ONLY CPU
    ### Process the provided parameters
    N_train, M_test, tau = data
    tau_txt = str(tau).replace('.', '')
    ...
    if sch:
        model_name = f"TrainedModels/{method}/Sym/{problem}/SaveAtEpochs/sch{problem}RandN{N_train}M{M_test}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_{nL}L{nN}n{nM}m"
    else:
        model_name = f"TrainedModels/{method}/Sym/{problem}/SaveAtEpochs/{problem}RandN{N_train}M{M_test}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{nL}L{nN}n{nM}m"
    ...
    models = torch.load(model_name + '/model')
    ...
    # Do all the models and their predictions and errors and everything
    epochs_used = []
    i = 0
    while i <= len(models):
        epochs_used.append(i)
        pred_inv = np.zeros([M+1, d])
        Z = torch.tensor(x0, dtype=torch.float64).reshape((1, 1, d))
        Tau = torch.tensor([[[tau**2]]], dtype=torch.float64)

        with torch.no_grad():
            inverse, _ = models[i].back(Z, Tau)

        pred_inv[0] = inverse.reshape((1, d)).numpy()

        for j in range(M):
            pred_inv[j+1] = numeric_stepNumpy(pred_inv[j], tau, extraParams)

        pred_inv = torch.from_numpy(np.float64(pred_inv)).reshape(M+1, 1, d)

        with torch.no_grad():
            pred, _ = models[i](pred_inv, Tau)

        pred = pred.reshape((M+1, d))

        abs_errors.append(mse_func(pred[-1], exact)) # Only abs error at Tend
        ham_errors.append(max(abs((probl_class.H(pred) -H0)/H0))) # Max abs ham error for the whole interval
        if problem == 'Kepler':
            ang_errors.append(max(abs((probl_class.L(pred) -L0)/L0))) # Max abs ham error for the whole interval
    ...
    # Plots global errors
    fig1, ax = plt.subplots(figsize=(9, 6.5))
    ax.plot(epochs_used, abs_errors, ls='-', color='k', linewidth='1', label='New method') 
    ax.set_xlabel("Epochs trained for")
    ax.set_ylabel("absolute error at Tend")
    ax.grid(True) 
    ax.set_title(f'Global error: L={str(nL)}, n={str(nN)}, m={str(nM)}')
    plt.savefig(model_name + f'/globalError_Skip{skip}', dpi=300, bbox_inches='tight')

    # Plots abs errors
    fig2, ax = plt.subplots(figsize=(9, 6.5))
    ax.plot(epochs_used, ham_errors, ls='-', color='k', linewidth='1', label='New method') 
    ax.set_xlabel("Epochs trained for")
    ax.set_ylabel("max abs error")
    ax.grid(True) 
    ax.set_title(f'Hamiltonian error: L={str(nL)}, n={str(nN)}, m={str(nM)}')
    plt.savefig(model_name + '/hamError_Skip{skip}', dpi=300, bbox_inches='tight')
    
    # Plots ang errors
    if problem == 'Kepler':
        fig3, ax = plt.subplots(figsize=(9, 6.5))
        ax.plot(epochs_used, ang_errors, ls='-', color='k', linewidth='1', label='New method') 
        ax.set_xlabel("Epochs trained for")
        ax.set_ylabel("max abs error")
        ax.grid(True) 
        ax.set_title(f'Angular momentum error: L={str(nL)}, n={str(nN)}, m={str(nM)}')
        plt.savefig(model_name + '/angError_Skip{skip}', dpi=300, bbox_inches='tight')
...
def model_conv(problem, method, device,  data, trainParams, nL, nN, nM, convParams, extraParams = None):
    ...
    # Save plots
    if linear:
        plot_name = model_name.replace(f'TrainedModels/{method}/Sym/{problem}/', f'TrainedModels/{method}/Sym/{problem}/ConvergenceGraphs/Linear/')
    else:
        plot_name = model_name.replace(f'TrainedModels/{method}/Sym/{problem}/', f'TrainedModels/{method}/Sym/{problem}/ConvergenceGraphs/Sigmoid/')
