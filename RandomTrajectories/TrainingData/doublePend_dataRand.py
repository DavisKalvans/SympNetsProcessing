...
N = 80 # Ammount of training data
train_X = np.zeros((N, 1, 4))
train_Y = np.zeros((N, 1, 4))
train_Tau = Tau*np.ones((N, 1, 1))
train_q1 = 3*np.random.rand(N, 1) - 1.5
train_q2 = 4*np.random.rand(N, 1) - 2
train_p1 = 4*np.random.rand(N, 1) - 2
train_p2 = 3*np.random.rand(N, 1) - 1.5
...
test_q2 = 4*np.random.rand(M, 1) - 2
test_p1 = 4*np.random.rand(M, 1) - 2
test_p2 = 3*np.random.rand(M, 1) - 1.5
...
for n in range(N):
    ...
    sol = solve_ivp(DoublePendulum, [0, tau], [train_q1[n].item(), train_q2[n].item(), train_p1[n].item(), train_p2[n].item()], 
...
for m in range(M):
    ...
    tau = test_Tau[m]
    # solve pendulum equations with RK45
    sol = solve_ivp(DoublePendulum, [0, tau], [test_q1[m].item(), test_q2[m].item(), test_p1[m].item(), test_p2[m].item()], 
                    method='RK45', rtol = 1e-12, atol = 1e-12) 
    # save data
    ...
    ax.plot(np.stack((test_X[m, 0, 0], test_Y[m, 0, 0])),
            np.stack((test_X[m, 0, 2], test_Y[m, 0, 2])), 
            ls='-', color='k', linewidth='1.5')
    ax.plot(test_X[m, 0, 0], test_X[m, 0, 2], marker='o', ms=7, mfc='g')
    ax.plot(test_Y[m, 0, 0], test_Y[m, 0, 2], marker='o', ms=7, mfc='y')

# Saving data
Tau_txt = str(Tau).replace('.', '')
np.savez(f'SavedTrainingData/DoublePend/DoublePendRandN{N}M{M}ConstTau{Tau_txt}', train_X=train_X, train_Y=train_Y, train_Tau=train_Tau, test_X=test_X, test_Y=test_Y, test_Tau=test_Tau)