...
M = 20 # Ammount of testing data data
...
ax.set_xlabel("$q$")
ax.set_ylabel("$p$")         
ax.set_title("Trenēšanas un validācijas datu kopas")  
ax.grid(True)
#ax.axis([-1.6, 1.6, -1, 1])
...
for n in range(N):
    ...
    ax.plot(train_X[n, 0, 0], train_X[n, 0, 1], marker='o', ms=7, mfc='b', mec ='b')
    ax.plot(train_Y[n, 0, 0], train_Y[n, 0, 1], marker='*', ms=7, mfc='b', mec ='b')
...
for m in range(M):
    ...
    ax.plot(np.stack((test_X[m, 0, 0], test_Y[m, 0, 0])),
            ...
            ls='-', color='k', linewidth='1.5')
    ax.plot(test_X[m, 0, 0], test_X[m, 0, 1], marker='o', ms=7, mfc='r', mec ='r')
    ax.plot(test_Y[m, 0, 0], test_Y[m, 0, 1], marker='*', ms=7, mfc='r', mec ='r')
