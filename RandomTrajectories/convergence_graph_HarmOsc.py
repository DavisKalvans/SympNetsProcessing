    ...
    for nN in [4, 8, 16, 32]: # Width of each layer
        ...
        for nM in [0]: # Seed
            ...
            for tau in taus:
                ...
                # Euler method
                for i in range(M):
                    pred_numeric[i+1, 1] = pred_numeric[i, 1] -omega**2*pred_numeric[i, 0]*tau
                    pred_numeric[i+1, 0] = pred_numeric[i, 0] + pred_numeric[i+1, 1]*tau

                predictions_numeric.append(torch.tensor(pred_numeric[-1]))
                energy_numeric = HarmOsc_H(pred_numeric)
                energies_numeric.append(max(energy_numeric))
                ...
                with torch.no_grad():
                    ...
                    energy_pred = HarmOsc_H(pred.reshape((M+1, d)))
                    energies_pred.append(max(energy_pred))

            ## Energy errors
            energies_numeric = np.abs(energies_numeric-H0)
            energies_pred = np.abs(energies_pred-H0)
            ...
            # Save plots
            if linear:
                plot_name = model_name.replace('TrainedModels/HarmOsc/', f'TrainedModels/HarmOsc/ConvergenceGraphs/Linear/')
            else:
                plot_name = model_name.replace('TrainedModels/HarmOsc/', f'TrainedModels/HarmOsc/ConvergenceGraphs/Sigmoid/')
            ...
            ax.loglog(np.array(taus), energies_pred, color = 'r', linewidth = '2', label = 'New method')
            ax.loglog(np.array(taus), energies_numeric, color = 'g', linewidth = '2', label = 'Euler')
            ax.loglog(np.array(taus), line1, color = 'k', label = "first order")
            ax.loglog(np.array(taus), line2, color = 'k', label = "second order")
            ax.legend(loc=4, prop={'size':20})
            ax.grid(True)
            ...
            ax.set_title(f"Harm_Osc L={nL}, N={nN}, M={nM}")
