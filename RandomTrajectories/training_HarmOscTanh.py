...
from NeuralNetwork.symp_module_class import SympGradModuleTanh
...
data = np.load(f'TrainingData/HarmOscRandN{N}M{M}ConstTau{tau_txt}.npz')
    ...
    for nN in [2, 4, 8, 16, 32]:
        ...
        for nM in [0]:    
            ...
            for nnet in [0]:
                ...
                layers = []
                for n in  range(nL):
                    layers.append(SympGradModuleTanh(d, nN, nL))
                ...
                for t in range(epochs):
                    loss[t] = train_loop(train_dataloader, model, omega,
                                        loss_fn, optimizer, scheduler, sch)
                    acc[t] = test_loop(test_dataloader, model, omega,
                                        loss_fn)
                ...
                if sch:
                    file_w = f'TrainedModels/HarmOsc/Tanh/schHarmOscRandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_' +f
                else:
                    file_w = f'TrainedModels/HarmOsc/Tanh/HarmOscRandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_' +f
                ...
                plt.yticks([10**(-9), 10**(-7), 10**(-5), 10**(-3), 10**(-1), 10**1])
                ax.set_title(f'HarmOsc $\omega$={omega}, L={nL}, N={nN}, m={nM}')
                # Save figure
                plt.savefig(file_w.replace('Tanh/', 'Tanh/MSE/') +".png", dpi=300, bbox_inches='tight')
                #plt.show()
