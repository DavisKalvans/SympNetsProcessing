...
from NeuralNetwork.training_class import train_loop_pendulum, test_loop_pendulum
...
data = np.load(f'TrainingData/SavedTrainingData/Pendulum/PendulumRandN{N}M{M}ConstTau{tau_txt}.npz')
        ...
        for nM in [0]:    
            ...
            for nnet in [0]:
                ...
                for t in range(epochs):
                    loss[t] = train_loop_pendulum(train_dataloader, model,
                                                loss_fn, optimizer, scheduler, sch)
                    acc[t] = test_loop_pendulum(test_dataloader, model, loss_fn)
                ...
                if sch:
                    file_w = f'TrainedModels/Pendulum/schPendulumRandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_' +f
                else:
                    file_w = f'TrainedModels/Pendulum/PendulumRandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_' +f
                ...
                # Save figure
                plt.savefig(file_w.replace('/Pendulum/', '/Pendulum/MSE/') +".png", dpi=300, bbox_inches='tight')
                #plt.show()

                del model

                ### Debug for getting training gradients
