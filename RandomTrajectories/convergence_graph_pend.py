...
from TrainingData.pendulum import Pendulum, Pendulum_H
...
exact = solve_ivp(Pendulum, [0, Tend], [q1, p1], method = 'RK45', rtol = 1e-12, atol = 1e-12)
...
H0 = Pendulum_H(np.array([q1, p1]).reshape((1, 2)))
...
for nL in [2, 4, 8]: # Number of layers
    ...
    for nN in [4, 8, 16, 32]: # Width of each layer
        ...
        for nM in [0]: # Seed
            ...
            if sch:
                model_name = f"TrainedModels/Pendulum/schPendulumRandN{N_train}M{M_test}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_{nL}L{nN}n{nM}m"
            else:
                model_name = f"TrainedModels/Pendulum/PendulumRandN{N_train}M{M_test}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{nL}L{nN}n{nM}m"
            
            if linear:
                model_name = model_name.replace('PendulumRand', 'PendulumLinearRand')
            ...
            for tau in taus:
                ...
                # Euler method
                for i in range(M):
                    pred_numeric[i+1, 1] = pred_numeric[i, 1] -np.sin(pred_numeric[i, 0])*tau
                    pred_numeric[i+1, 0] = pred_numeric[i, 0] + pred_numeric[i+1, 1]*tau

                predictions_numeric.append(torch.tensor(pred_numeric[-1]))
                energy_numeric = Pendulum_H(pred_numeric)
                ...
                with torch.no_grad():
                    ...
                    a = inverse[:, 0, 1] -Tau.mT*torch.sin(inverse[:, 0, 0])
                    ...
                    for m in range(M-1):
                        pred_inv[m+2, 1] = pred_inv[m+1, 1] -tau*np.sin(pred_inv[m+1, 0])
                    ...
                    energy_pred = Pendulum_H(pred.reshape((M+1, d)))
                    energies_pred.append(energy_pred)

            ### Calculate errors for our predictions at endpoint
            # and energy errors as max deviation in the whole interval [0, Tend]
            ...
            ax.set_title(f"Pendulum L={nL}, N={nN}, M={nM}")

            # Save plots
            if linear:
                plot_name = model_name.replace('TrainedModels/Pendulum/', f'TrainedModels/Pendulum/ConvergenceGraphs/Linear/')
            else:
                plot_name = model_name.replace('TrainedModels/Pendulum/', f'TrainedModels/Pendulum/ConvergenceGraphs/Sigmoid/')
            plot_name = plot_name.replace('_T1000Alternate.npy', '')

            plt.savefig(plot_name + '_absolute', dpi=300, bbox_inches='tight')

            # Plots absolute errors
