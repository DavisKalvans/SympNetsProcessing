...
from NeuralNetwork.training_class import train_loop_classic, test_loop_classic
        ...
        for nM in [0]:    
            ...
            for nnet in [0]:
                ...
                for t in range(epochs):
                    loss[t] = train_loop_classic(train_dataloader, model,
                                        loss_fn, optimizer, scheduler, sch)
                    acc[t] = test_loop_classic(test_dataloader, model,
                                        loss_fn)
                ...
                if sch:
                    file_w = f'TrainedModels/HarmOsc/schHarmOscNormalRandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_' +f
                else:
                    file_w = f'TrainedModels/HarmOsc/HarmOscNormalRandN{N}M{M}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_' +f
                ...
                # Save figure
                plt.savefig(file_w.replace('/HarmOsc/', '/HarmOsc/MSE/') +".png", dpi=300, bbox_inches='tight')
                #plt.show()
