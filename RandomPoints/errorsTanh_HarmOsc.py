...
sol = np.load(f'TrainingData/Analytical/HarmOsc/Const{tau_txt}Tau_T100.npy')
numeric = np.load(f'TrainingData/SavedTrainingData/HarmOsc/Euler_Const{tau_txt}Tau_T100.npy')
...
for nL in [2]: # Number of layers
    ...
    for nN in [2, 4, 8, 16, 32]: # Width of each layer
        ...
        for nM in [0]: # Seed
            ...
            if sch:
                model_name = f"TrainedModels/HarmOsc/Tanh/Predictions/schHarmOscRandN{N_train}M{M_test}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_{nL}L{nN}n{nM}m_T100Alternate.npy"
            else:
                model_name = f"TrainedModels/HarmOsc/Tanh/Predictions/HarmOscRandN{N_train}M{M_test}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{nL}L{nN}n{nM}m_T100Alternate.npy"
            ...
            # Plot errors
            plot_name = model_name.replace('Predictions/', f'Predictions/T{str(Tend)}/')
            plot_name = plot_name.replace('_T100Alternate.npy', '')
