...
from TrainingData.pendulum import Pendulum_H
...
sol = np.load(f'TrainingData/Analytical/Pendulum/Const{tau_txt}Tau_T1000.npy')
numeric = np.load(f'TrainingData/Numeric/Pendulum/Euler_Const{tau_txt}Tau_T1000.npy')

Tend = 100
M = int(Tend/tau) # Time steps
tm = np.linspace(0, Tend, M)

for nL in [8]: # Number of layers
    ...
    for nN in [4, 8, 16, 32]: # Width of each layer
        ...
        for nM in [0]: # Seed
            ...
            if sch:
                model_name = f"TrainedModels/Pendulum/Predictions/schPendulumRandN{N_train}M{M_test}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_{nL}L{nN}n{nM}m_T1000Alternate.npy"
            else:
                model_name = f"TrainedModels/Pendulum/Predictions/PendulumRandN{N_train}M{M_test}Const{tau_txt}Tau{epochs_th}TH_{eta1_txt}eta1_{nL}L{nN}n{nM}m_T1000Alternate.npy"
            ...
            for k in range(Mn): # Go trough all the trajectories
                ...
                H0 = Pendulum_H(pred_tmp[0, 0:2].reshape((1, 2)))
                Err += np.sqrt(np.sum((pred_tmp -sol_tmp)**2, 1)).reshape((M, 1))
                HErr += np.abs((Pendulum_H(pred_tmp).reshape((M, 1)) -H0)/H0)

                Err_numeric += np.sqrt(np.sum((numeric_tmp -sol_tmp)**2, 1)).reshape((M, 1))
                HErr_numeric += np.abs((Pendulum_H(numeric_tmp).reshape((M, 1)) -H0)/H0)
            ...
            ax.set_xlabel("$q1$")
            ax.set_ylabel("$p1$")
            ax.set_title(f'Pendulum: L={str(nL)}, n={str(nN)}, m={str(nM)}')
