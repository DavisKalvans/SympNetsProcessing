...
from NeuralNetwork.training_class import train_loopGeneralSym, test_loopGeneralSym
...
method = 'Verlet'
device = 'cpu'
data = (40, 20, 0.1, 0.001, 0.1) # Training data N, testing data M, tau (for predictions), min timestep, max timestep
nL = 8 # Layers
...
convParams = (x0_Pendulum, 10, 3, 14, 4, False) # x0, Tend, lower power, upper power, dim, best
...
def train_model(problem, method, device, data, trainParams, nL, nN, nM, extraParams = None):
    ...
    ### Load training data
    N, M, tau, tau_min, tau_max = data
    min_txt = str(tau_min).replace('.', '')
    max_txt = str(tau_max).replace('.', '')

    npz_file = np.load(f'TrainingData/SavedTrainingData/{problem}/{problem}RandN{N}M{M}tauMin{min_txt}tauMax{max_txt}.npz')
    ...
    sigma = np.sqrt(1) # Does this being sqrt(0.01) ruin everything?
    ...
    for t in range(epochs):
        loss[t] = train_loopGeneralSym(numeric_step, train_dataloader, model, loss_fn, optimizer, scheduler, sch, params)
        acc[t] = test_loopGeneralSym(numeric_step, test_dataloader, model, loss_fn, params)
    ...
    if sch:
        model_name = f'TrainedModels/{method}/Sym/diffTau/{problem}/sch{problem}RandN{N}M{M}minTau{min_txt}maxTau{max_txt}_{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_' +f
    else:
        model_name = f'TrainedModels/{method}/Sym/diffTau/{problem}/{problem}RandN{N}M{M}minTau{min_txt}maxTau{max_txt}_{epochs_th}TH_{eta1_txt}eta1_' +f
...
def model_predictions(problem, method, device, data, trainParams, nL, nN, nM, predParams, extraParams = None):

    ### Process the provided parameters
    N_train, M_test, tau, tau_min, tau_max = data
    min_txt = str(tau_min).replace('.', '')
    max_txt = str(tau_max).replace('.', '')

    M, x0, best = predParams
    Tend = tau*M
    ...
    if sch:
        model_name = f"TrainedModels/{method}/Sym/diffTau/{problem}/sch{problem}RandN{N_train}M{M_test}minTau{min_txt}maxTau{max_txt}_{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_{nL}L{nN}n{nM}m"
    else:
        model_name = f"TrainedModels/{method}/Sym/diffTau/{problem}/{problem}RandN{N_train}M{M_test}minTau{min_txt}maxTau{max_txt}_{epochs_th}TH_{eta1_txt}eta1_{nL}L{nN}n{nM}m"
    ...
    file_name = model_name.replace(f"TrainedModels/{method}/Sym/diffTau/{problem}/", f"TrainedModels/{method}/Sym/diffTau/{problem}/Predictions/")
...
def model_errors(problem, method, data, trainParams, nL, nN, nM, plotParams, extraParams = None):
    ...
    if sch:
        model_name = f"TrainedModels/{method}/Sym/diffTau/{problem}/Predictions/sch{problem}RandN{N_train}M{M_test}minTau{min_txt}maxTau{max_txt}_{epochs_th}TH_{eta1_txt}eta1_{eta2_txt}eta2_{nL}L{nN}n{nM}m_T1000Alternate.npy"
    else:
        model_name = f"TrainedModels/{method}/Sym/diffTau/{problem}/Predictions/{problem}RandN{N_train}M{M_test}minTau{min_txt}maxTau{max_txt}_{epochs_th}TH_{eta1_txt}eta1_{nL}L{nN}n{nM}m_T1000Alternate.npy"
    ...
    ### Load analytical result for error calculations and numeric for error comparison
    sol = np.load(f'TrainingData/Analytical/{problem}/Const{tau_txt}Tau_T1000.npy')
    numeric = np.load(f'TrainingData/Numeric/{problem}/{method}_Const{tau_txt}Tau_T1000.npy')

    Mn = 1 # Number of trajectories - NOT IMPLEMENTED TODO

    ### Absolute and Hamiltonian errors
    Err = np.zeros([M, 1])
    HErr = np.zeros([M, 1])
    Err_numeric = np.zeros([M, 1])
    HErr_numeric = np.zeros([M, 1]) 
    
    for k in range(Mn): # Go trough all trajectories
        sol_tmp = sol.T[0:M, :]
        ...
        H0 = probl_class.H(sol_tmp[0, :].reshape((1, d)), extraParams)
        HErr += np.abs((probl_class.H(pred_tmp, extraParams).reshape((M, 1)) -H0)/H0)
        HErr_numeric += np.abs((probl_class.H(numeric_tmp, extraParams).reshape((M, 1)) -H0)/H0)
    ...
    if problem == 'Kepler':
        ...
        for k in range(Mn): # Go trough all trajectories
            ...
            L0 = probl_class.L(sol_tmp[0, 0:d].reshape((1, d)), extraParams)
            LErr += np.abs((probl_class.L(pred_tmp, extraParams).reshape((M, 1)) -L0)/L0)
            LErr_numeric += np.abs((probl_class.L(numeric_tmp, extraParams).reshape((M, 1)) -L0)/L0)

    Tend_txt = str(Tend).replace('.0', '')
    ### Plot the errors
    if linear:
        plot_name = model_name.replace('Predictions/', f'Predictions/Linear/T{Tend_txt}/')
    else:
        plot_name = model_name.replace('Predictions/', f'Predictions/Sigmoid/T{Tend_txt}/')
    ...
    ax.plot(tm, Err_numeric/Mn, ls='-', color='g', linewidth='1', label=f'{method} method') 
    ...
    ax.plot(tm, HErr_numeric/Mn, ls='-', color='g', linewidth='1', label=f'{method} method') 
    ax.legend(loc=1, prop={'size':20})
    ax.set_xlabel("time")
    ...
    if problem == 'Kepler':
        ...
        ax.plot(tm, LErr_numeric/Mn, ls='-', color='g', linewidth='1', label=f'{method} method') 
        ax.legend(loc=1, prop={'size':20})
        ax.set_xlabel("time")
        ax.set_ylabel("absolute relative error")
        ax.grid(True) 
    ...
    ax.plot(numeric_tmp[:, 0], numeric_tmp[:, 1], ls='--', color='g', linewidth='1', label=f'{method} method')
    ax.legend(loc=1, prop={'size':20})
...
def model_multTrajectErrors(problem, method, device, data, trainParams, nL, nN, nM, predParamsMult, extraParams = None):
    ...
    for k in range(nr_trajects):
        exact = solve_ivp(probl_class.problem, [0, Tend], x0[k], args = (extraParams,), t_eval=tm, method = 'RK45', rtol = 1e-12, atol = 1e-12)
        exact = exact.y.T
        exact = np.array(exact)
        exacts.append(exact)

    #return exacts, predictions, tm
    ### Absolute and Hamiltonian errors
    Err = np.zeros([M+1, 1])
    HErr = np.zeros([M+1, 1])
    ...
    if linear:
        plot_name = model_name.replace(f'Sym/{problem}/', f'Sym/{problem}/PredictionsMultiple/Linear/')
    else:
        plot_name = model_name.replace(f'Sym/{problem}/', f'Sym/{problem}/PredictionsMultiple/Sigmoid/')

    plot_name = plot_name +f'_seed{seed}_nr{nr_trajects}_T{Tend_txt}'

    # Plots global error
    fig1, ax = plt.subplots(figsize=(9, 6.5))
    ax.plot(tm, Err/nr_trajects, ls='-', color='k', linewidth='1', label='New method') 
    ...
    # Plots Hamiltonian error
    fig2, ax = plt.subplots(figsize=(9, 6.5))
    ax.plot(tm, HErr/nr_trajects, ls='-', color='k', linewidth='1', label='New method') 
    ...
    if problem == 'Kepler':
        fig3, ax = plt.subplots(figsize=(9, 6.5))
        ax.plot(tm, LErr/nr_trajects, ls='-', color='k', linewidth='1', label='New method') 
...
def model_conv(problem, method, device,  data, trainParams, nL, nN, nM, convParams, extraParams = None):
    ...
    exact = solve_ivp(probl_class.problem, [0, Tend], x0, args = (extraParams,), method = 'RK45', rtol = 1e-12, atol = 1e-12)
    ...
    # Save plots
    if linear:
        plot_name = model_name.replace(f'TrainedModels/{method}/Sym/diffTau/{problem}/', f'TrainedModels/{method}/Sym/diffTau/{problem}/ConvergenceGraphs/Linear/')
    else:
        plot_name = model_name.replace(f'TrainedModels/{method}/Sym/diffTau/{problem}/', f'TrainedModels/{method}/Sym/diffTau/{problem}/ConvergenceGraphs/Sigmoid/')
...
def model_multTrajectConv(problem, method, device,  data, trainParams, nL, nN, nM, convParamsMult, extraParams = None):
    ...
    for i in range(nr_trajects):
        exact = solve_ivp(probl_class.problem, [0, Tend], x0[i], args = (extraParams,), method = 'RK45', rtol = 1e-12, atol = 1e-12)
        exact = exact.y.T[-1] # Only want the solution at time t=Tend
        exact = np.array(exact)
        exact = torch.tensor(exact, dtype = torch.float64, device = device).reshape((1, d))
        exacts.append(exact)
    ...
    ### Plot absolute errors
    fig1, ax = plt.subplots(figsize=(9, 6.5))
    ax.loglog(np.array(taus), errors/nr_trajects, color = 'r', linewidth = '2', label = 'New method')
    ...
    # Save plots
    if linear:
        plot_name = model_name.replace(f'TrainedModels/{method}/Sym/{problem}/', f'TrainedModels/{method}/Sym/{problem}/ConvergenceGraphsMultiple/Linear/')
    else:
        plot_name = model_name.replace(f'TrainedModels/{method}/Sym/{problem}/', f'TrainedModels/{method}/Sym/{problem}/ConvergenceGraphsMultiple/Sigmoid/')
    ...
    ### Plots hamiltonian errors
    fig2, ax = plt.subplots(figsize=(9, 6.5))
    ax.loglog(np.array(taus), (errors_energy/nr_trajects), color = 'r', linewidth = '2', label = 'New method')
    ...
    if problem == 'Kepler':
        fig3, ax = plt.subplots(figsize=(9, 6.5))
        ax.loglog(np.array(taus), (errors_ang/nr_trajects), color = 'r', linewidth = '2', label = 'New method')
